...
### Added
- Sort conversations by **Messages** (highest message count first)
- Capture MMS `m_size` attribute during import for more accurate conversation size calculations (includes media size, not just text)
- Schema migration to add `m_size` column for existing databases

### Changed
- Conversation size calculation now uses MMS `m_size` when available, falling back to text body length for SMS

## [1.0.4] - 2026-03-22

### Added
- Per-conversation total message body size displayed as a badge (e.g. "1.2 MB") in the conversation list
- Sort conversations by **Size** (largest first) or **Recent** (default) via toggle buttons above the filter bar
- `totalBodySize` field on `Conversation` type computed via SQL `SUM(LENGTH(body))`

## [1.0.3] - 2026-03-22

### Added
- Comprehensive README.md with setup instructions, usage guide, project structure, architecture overview, and tech stack — useful for both human users and AI agents

## [1.0.2] - 2026-01-14

### Changed
- **MAJOR:** Replaced SAX streaming parser with fast-xml-parser for SMS/MMS parsing
  - More robust XML parsing with proper standards compliance
  - Eliminates buffer overflow issues with large message bodies
  - No more "Max buffer length exceeded" errors
  - Handles edge cases (entities, CDATA, attributes) correctly
- **MAJOR:** Replaced SAX streaming parser with fast-xml-parser for call log parsing
  - Same benefits as SMS parser rewrite
  - Simpler, more maintainable code

### Removed
- Removed `sax` dependency - no longer needed after fast-xml-parser migration
...
[1.0.2]: https://github.com/yourusername/phone-sms-viewer/releases/tag/v1.0.2
